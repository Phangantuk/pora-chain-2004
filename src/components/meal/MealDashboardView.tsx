    ...
    <div className={compact ? 'px-6 lg:px-8 py-8' : 'max-w-6xl mx-auto px-6 py-12'}>
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between mb-8">
        ...
        <div className="flex flex-wrap gap-2">
          <Link href={lp('/meal/donate')} className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-[#E8855A] text-[#0D0805] font-semibold text-[13px] hover:bg-[#f0966e] transition-all duration-200">
            + {m.ctaDonate}
          </Link>
          <Link href={lp('/meal/explorer')} className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl border border-white/[0.08] text-white/55 font-medium text-[13px] hover:text-white/80 hover:border-white/[0.16] transition-all duration-200">
            {m.ctaExplorer}
          </Link>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-[1.35fr_1fr] gap-6 mb-6">
        <div className="bg-[#0C0C0E] border border-white/[0.07] rounded-2xl overflow-hidden">
          <div className="px-5 py-4 border-b border-white/[0.05] flex items-center justify-between">
            ...
            <Link href={lp('/meal/donate')} className="font-mono text-[11px] text-white/25 hover:text-[#E8855A] transition-colors inline-flex items-center gap-1.5">
            ...
            {MOCK_USER_DONATIONS.map((donation) => (
              <div key={donation.id} className="px-5 py-4 flex items-center justify-between gap-3">
                ...
                <div className="text-right">
                  ...
                  <span className={`font-mono text-[10px] ${donation.status === 'confirmed' ? 'text-[#4ECAA0]' : 'text-[#F5C542]'}`}>
        ...
        <div className="bg-[#0C0C0E] border border-white/[0.07] rounded-2xl overflow-hidden">
          <div className="px-5 py-4 border-b border-white/[0.05]">
            <p className="font-mono text-[10px] tracking-[0.12em] uppercase text-white/25">{m.dashImpactEvents}</p>
          </div>
          <div className="divide-y divide-white/[0.04]">
            {recentEvents.map((event) => (
              <div key={event.id} className="px-5 py-4">
                ...
                <div className="flex items-center gap-2.5 font-mono text-[11px] text-white/25">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {[
          ...
          <Link key={item.href} href={item.href} className="group bg-[#0C0C0E] border border-white/[0.07] rounded-2xl p-5 hover:border-white/[0.14] transition-colors no-underline">
            <p className="font-semibold text-[14px] text-white/85 mb-2 group-hover:text-white">{item.title}</p>
            <p className="text-[13px] text-white/38 font-light leading-relaxed mb-4">{item.text}</p>
            <span className="font-mono text-[11px] text-[#E8855A]/65 group-hover:text-[#E8855A] transition-colors inline-flex items-center gap-1.5">
              <span>{xt.meal.openAction}</span>
              <RightArrow />
            </span>
          </Link>
        ))}

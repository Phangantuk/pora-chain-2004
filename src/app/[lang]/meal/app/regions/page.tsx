...
export default function AppRegionsPage({ params }: { params: { lang: string } }) {
  ...
  const x = {
    myRegions: lang === 'ru' ? 'Мои регионы' : lang === 'es' ? 'Mis regiones' : 'My regions',
    browseAll: lang === 'ru' ? 'Все регионы' : lang === 'es' ? 'Todas las regiones' : 'Browse all',
    suggested: lang === 'ru' ? 'Рекомендуемые регионы' : lang === 'es' ? 'Regiones sugeridas' : 'Suggested regions',
    ofGoal: lang === 'ru' ? 'от цели' : lang === 'es' ? 'de la meta' : 'of goal',
  }
  ...
  return (
    <div className="px-6 lg:px-8 py-8">
      <div className="flex items-center justify-between mb-8 gap-3">
        <h1 className={`font-bold text-white ${compactRu ? 'text-[20px]' : 'text-[22px]'}`}>{x.myRegions}</h1>
        <Link href={lp('/meal/regions')} className="font-mono text-[12px] text-white/30 hover:text-[#E8855A] transition-colors inline-flex items-center gap-1.5">
          <span>{x.browseAll}</span>
          <RightArrow />
        </Link>
      </div>
      ...
      <div className="flex flex-col gap-4 mb-10">
        {followed.map((region) => {
          ...
          return (
            <div
              key={region.slug}
              className="bg-[#0C0C0E] border border-white/[0.07] rounded-2xl p-6"
              style={{
                background: `linear-gradient(#0C0C0E,#0C0C0E) padding-box, linear-gradient(135deg,${color}15,transparent 60%) border-box`,
            ...
            >
              <div className="flex flex-wrap items-start justify-between gap-4 mb-4">
                ...
                <div className="flex gap-2 shrink-0">
                  <Link
                    href={lp(`/meal/regions/${region.slug}`)}
                    className={`font-mono rounded-lg border border-white/[0.08] text-white/35 hover:text-white/65 transition-all ${compactRu ? 'text-[10.5px] px-2.5 py-1.5' : 'text-[11px] px-3 py-1.5'}`}
                  >
                    {m.viewRegion}
                  </Link>
                  <Link
                    href={lp(`/meal/donate?region=${region.slug}`)}
                    className={`font-mono rounded-lg bg-[#E8855A] text-[#0D0805] font-semibold hover:bg-[#f0966e] transition-all ${compactRu ? 'text-[10.5px] px-2.5 py-1.5' : 'text-[11px] px-3 py-1.5'}`}
                  >
                    {m.donate}
                  </Link>
                </div>
              ...
              <div>
                <div className="flex justify-between text-[11px] font-mono text-white/25 mb-1.5 gap-2">
                  <span>{m.raised}: {formatUsd(region.raisedUsd)}</span>
                  <span>{pct}% {x.ofGoal}</span>
                </div>
      ...
      <p className="font-mono text-[10px] tracking-[0.12em] uppercase text-white/20 mb-4">{x.suggested}</p>

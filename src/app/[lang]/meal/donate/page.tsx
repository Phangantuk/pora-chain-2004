...
const METHODS = [
  { id: 'usdt', label: 'USDT TRC20', icon: '💎', network: 'TRON', note: 'Fastest, lowest fees' },
  { id: 'eth',  label: 'ETH / ERC20', icon: '⟠',  network: 'Ethereum', note: 'ERC20 tokens supported' },
  { id: 'fiat', label: 'Bank / Card', icon: '💳',  network: 'Coming soon', note: 'Fiat onramp coming soon', disabled: true },
]
...
export default function DonatePage({ params }: { params: { lang: string } }) {
  ...
  const preselected   = searchParams.get('region') ?? ''

  const [region,   setRegion]   = useState(preselected)
  const [amount,   setAmount]   = useState('100')
  const [method,   setMethod]   = useState('usdt')
  const [step,     setStep]     = useState<'form' | 'confirm' | 'success'>('form')
  const [loading,  setLoading]  = useState(false)

  useEffect(() => { if (preselected) setRegion(preselected) }, [preselected])
      ...
      <div className="bg-[#070707] text-white min-h-screen">
        ...
        <div className="max-w-lg mx-auto px-6 pt-24 pb-20 text-center">
          ...
          <div className="flex flex-col gap-3">
            <Link href={lp('/meal/explorer')}
              className="block w-full py-3 rounded-xl bg-[#E8855A] text-[#0D0805] font-semibold text-[14px] hover:bg-[#f0966e] transition-all duration-200 text-center">
              {m.ctaExplorer}
            </Link>
            <Link href={lp('/meal/regions')}
              className="block w-full py-3 rounded-xl border border-white/[0.08] text-white/50 font-medium text-[14px] hover:text-white/80 transition-all duration-200 text-center">
              {m.ctaRegions}
            </Link>
          </div>
        </div>
      ...
      <div className="max-w-2xl mx-auto px-6 py-10">
        ...
        {step === 'form' ? (
          <div className="flex flex-col gap-8">
            ...
            <div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {REGIONS.map(r => {
                  const pct = Math.min(100, Math.round((r.raisedUsd / r.goalUsd) * 100))
                    ...
                    <button key={r.slug} onClick={() => setRegion(r.slug)}
                      ...
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        <div className="flex-1 min-w-0">
                          <p className={`text-[13px] font-semibold truncate ${active ? 'text-white' : 'text-white/70'}`}>{r.name}</p>
                          <p className="font-mono text-[10px] text-white/25">{r.country}</p>
                        </div>
                        {r.status === 'urgent' && <span className="font-mono text-[9px] text-[#F5C542] bg-[#F5C542]/10 border border-[#F5C542]/25 px-1.5 py-0.5 rounded shrink-0">{m.urgent}</span>}
                      </div>
              ...
              <div className="flex flex-wrap gap-2 mb-3">
                {PRESET_AMOUNTS.map(a => (
                  <button key={a} onClick={() => setAmount(String(a))}
                    className={`font-mono text-[13px] px-4 py-2 rounded-lg border transition-all duration-200 ${amount === String(a) ? 'border-[#E8855A]/50 bg-[#E8855A]/[0.08] text-[#E8855A]' : 'border-white/[0.07] text-white/45 hover:border-white/[0.15]'}`}>
                    ${a}
                  </button>
                ))}
              </div>
            ...
            <div>
              ...
              <div className="flex flex-col gap-2">
                {METHODS.map(opt => {
                  ...
                  return (
                    <button key={opt.id} onClick={() => !opt.disabled && setMethod(opt.id)} disabled={opt.disabled}
                      className={`flex items-center gap-4 text-left rounded-xl border p-4 transition-all duration-200 ${opt.disabled ? 'opacity-40 cursor-not-allowed border-white/[0.05]' : active ? 'border-[#E8855A]/50 bg-[#E8855A]/[0.06]' : 'border-white/[0.07] bg-[#0C0C0E] hover:border-white/[0.14]'}`}>
                      <span className="text-[22px] shrink-0">{opt.icon}</span>
                      <div className="flex-1">
                        <p className={`font-semibold text-[14px] ${active ? 'text-white' : 'text-white/70'}`}>{opt.label}</p>
                        <p className="font-mono text-[10px] text-white/30">{opt.network} · {opt.note}</p>
                      </div>
                      {active && (
          ...
          <div className="max-w-lg mx-auto">
            ...
            <p className="text-[13px] text-white/30 font-light leading-relaxed mb-6">
              {m.confirmNote}
            </p>
